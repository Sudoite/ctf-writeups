...
long get_long() {
    ...
    while(!is_digit(c))
      ...
      while(c != '\n')
        c = getchar();
    printf("In get_long: returning l = %lu", l);
    return l;
}
...
void play_roulette(long choice, long bet) {
  ...
  if (spin == choice) {
    //printf("Cash before adding bet = $%lu.\n", cash);
    cash += 2*bet;
    //printf("Cash after adding bet = $%lu.\n", cash);
...
int main(int argc, char *argv[]) {
  ...
  while(cash > 0) {
      ...
      //printf("Cash after playing roulette as unsigned long = $%lu.\n", cash);
      //printf("Cash after playing roulette as signed long = $%ld.\n", cash);
      if (wins >= MAX_WINS) {
